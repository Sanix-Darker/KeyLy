...
int main(int argc, char *argv[]){
	...
	cout<<"\nNB: Don't close this console, swipe it down(en francais facile abaisse juste et travaille normalement, tout ce que tu fais est dans un fichier .log)";
	puts(""); // LOL, sans deconner
	...
	while(1){
		...
		if(lastTitle != title){
			klogout << endl << endl << "Programme: ";
#ifdef DEBUG
			cout << endl << endl << "Programme: ";
#endif
			if(strlen(title) == 0){
				klogout << "AUCUNE FENETRE ACTIVE";
#ifdef DEBUG
				cout << "AUCUNE FENETRE ACTIVE";
#endif
			}
			else{
				klogout << "'" << title << "'";
